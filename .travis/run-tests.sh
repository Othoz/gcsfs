...
if [ "$TRAVIS_PULL_REQUEST" = "false" ]; then
  ...
  openssl aes-256-cbc -K "$encrypted_6428cfa12bba_key" -iv "$encrypted_6428cfa12bba_iv" -in .travis/service-account-key.json.enc -out "$HOME"/service-account-key.json -d
